...
export default function Header() {
    ...
    return (
        <header className="fixed top-3 sm:top-5 left-0 right-0 z-50 px-3 sm:px-6 lg:px-8 pointer-events-none transition-all duration-300">
            ...
            <div className="w-full max-w-[96%] 2xl:max-w-[1600px] mx-auto bg-[#eef3f9]/95 backdrop-blur-xl border border-slate-200/80 rounded-full px-5 sm:px-8 py-3 flex items-center justify-between pointer-events-auto transition-all duration-300">
                ...
                <nav className="hidden lg:flex items-center gap-1">
                    {navLinks.map((link) => (
                        ...
                        >
                            <div className="flex items-center">
                                <Link
                                    ...
                                    className={`font-display flex items-center gap-1.5 px-4 py-2 text-[13.5px] font-semibold transition-colors duration-150 rounded-full ${
                                        pathname === link.href || (link.children && pathname.startsWith('/products'))
                                    ...
                                    {link.children && (
                                        <ChevronDown
                                            ...
                                            className={`text-[#6b7280] transition-transform duration-200 ${
                                                activeDropdown === link.label ? "rotate-180 text-[#3b82f6]" : ""
                                            }`}
                                        />
                                    )}
                                </Link>
                            ...
                            {/* Dropdown with seamless padding bridge */}
                            {link.children && (
                                <div
                                    ...
                                    onMouseLeave={handleMouseLeave}
                                >
                                    <div className="bg-white/95 backdrop-blur-xl border border-blue-200/80 rounded-2xl shadow-xl shadow-slate-200/60 p-2 space-y-1">
                                        {link.children.map((child) => (
                                            <Link
                                                key={child.label}
                                                href={child.href}
                                                onClick={() => setActiveDropdown(null)}
                                                className={`flex items-center px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-150 ${
                                                    pathname === child.href
                                                        ? "bg-blue-50 text-[#3b82f6]"
                                                        : "text-[#0f1f3d] hover:text-[#3b82f6] hover:bg-[#eef3f9]"
                                                }`}
                                            >
                                                {child.label}
                                            </Link>
                ...
                <div className="bg-[#eef3f9]/95 backdrop-blur-xl border border-blue-200/70 rounded-[28px] shadow-2xl p-5 space-y-2">
                    {navLinks.map((link) => (
                        <div key={link.label}>
                            ...
                            {link.children && (
                                <div className="ml-4 mt-1 space-y-1 border-l-2 border-blue-200/60 pl-3">
                                    {link.children.map((child) => (
                                        <Link
                                            key={child.label}
                                            href={child.href}
                                            onClick={() => setIsOpen(false)}
                                            className="flex items-center px-3 py-2 rounded-xl text-xs font-bold text-[#0f1f3d] hover:text-[#3b82f6] hover:bg-white/60 transition-colors"
                                        >
                                            {child.label}
                                        </Link>
                                    ))}
